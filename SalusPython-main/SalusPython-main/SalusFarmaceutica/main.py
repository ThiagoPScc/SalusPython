...
def calcular_frete():
    ...
    dlat = math.radians(lat - latFarma)
    dlon = math.radians(lon - lonFarma)

    a = math.sin(dlat / 2) ** 2 + math.cos(math.radians(latFarma)) * math.cos(math.radians(lat)) * math.sin(
        dlon / 2) ** 2
    ...
    return render_template('cart.html', cart=cart, total=total, frete=frete)
